...
# --------------------------------------------------------------- app.ico ---
#
# The Windows shell reads the exe's icon resource, not ``app.svg``; the ``.ico``
# is generated by ``scripts/make_icon.py`` and committed so that a plain
# ``pyinstaller qtRequestory.spec`` needs no Qt pre-step. Nothing at runtime
# opens it, so a container Windows silently refuses (a missing size, a wrong
# offset) would only show up as a blank icon on a colleague's taskbar.

ICO = PACKAGE / "ui" / "icons" / "app.ico"
EXPECTED_ICON_SIZES = {16, 24, 32, 48, 256}
